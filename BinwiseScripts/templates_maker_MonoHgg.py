...
class TemplatesApp(PlotApp):
    ...
    def __init__(self,option_list=[],option_groups=[]):
        ...
        super(TemplatesApp,self).__init__(option_groups=[
                ( "Fit definition options. Usually specified through JSON configuration (see templates_maker.json for details)", [
                        ...
                        make_option("--prepare-nohiggs",dest="prep_nohiggs",action="store_true",
                                    default=False,help="prepare templates without SM H->gg"),
                        make_option("--prepare-noggh",dest="prep_noggh",action="store_true",
                                    default=False,help="prepare templates without ggH->gg"),
                        make_option("--prepare-novbf",dest="prep_novbf",action="store_true",
                                    default=False,help="prepare templates without VBF->gg"),
                        make_option("--prepare-novh",dest="prep_novh",action="store_true",
                                    default=False,help="prepare templates without VH->gg"),
                        make_option("--prepare-notth",dest="prep_notth",action="store_true",
                                    default=False,help="prepare templates without ttH->gg"),
    ...
    def prepareTemplates(self,options,args):
        ...
        tmp = fout
        ## read input trees
        print  "\n Taking from json file \n options.data_file = %s \n options.data =%s \n options.infile = %s \n options.templates = %s\n options.prep_data = %s \n options.mc_file = %s\n options.mc = %s\n options.templatesMC = %s \n options.prep_nosig = %s \n options.prep_nohiggs = %s \n options.prep_noggh = %s \n options.prep_novbf = %s \n options.prep_novh = %s \n options.prep_notth = %s \n" % (options.data_file, options.data, options.infile, options.templates, options.prep_data, options.mc_file, options.mc, options.templatesMC, options.prep_nosig, options.prep_nohiggs, options.prep_noggh, options.prep_novbf, options.prep_novh, options.prep_notth)
        ...
        if not (options.prep_data or options.prep_nohiggs):
            for name,trees in options.higgs.iteritems():
                self.datasets_[name] = self.openDataset(None,options.mc_file,options.infile,trees)

        if not (options.prep_data or options.prep_noggh):
            for name,trees in options.ggh.iteritems():
                self.datasets_[name] = self.openDataset(None,options.mc_file,options.infile,trees)

        if not (options.prep_data or options.prep_novbf):
            for name,trees in options.vbf.iteritems():
                self.datasets_[name] = self.openDataset(None,options.mc_file,options.infile,trees)

        if not (options.prep_data or options.prep_novh):
            for name,trees in options.vh.iteritems():
                self.datasets_[name] = self.openDataset(None,options.mc_file,options.infile,trees)

        if not (options.prep_data or options.prep_notth):
            for name,trees in options.tth.iteritems():
                self.datasets_[name] = self.openDataset(None,options.mc_file,options.infile,trees)

        print "self.datsets_ = %s \n" % self.datasets_
        
            # used by parent class PlotApp to read in objects
        self.template_ = options.treeName
        print "self.template_ = %s" % self.template_

        self.groups = options.groups
        print "self.groups = %s" % options.groups
        if options.groups:
            self.categories_ = options.groups.keys()
        else:
            self.categories_ = options.categories

        print "self.categories_ = %s" % self.categories_

        ## create output workspace
        ## self.workspace_.rooImport = getattr(self.workspace_,"import")
        self.workspace_.rooImport = rooImport(self.workspace_)
        print "self.workspace_ = %s" % self.workspace_
        ## read and store list of aliases. will be defined later in all trees
        print "options.aliases = %s" %options.aliases
        print "seelf.aliases_ = %s" %self.aliases_
        for var in options.aliases:
            print "var = %s" %var            
            self.getVar(var)
            
        print "now self.aliases_ = %s " % self.aliases_

        ## define list of variables for the dataset
        varlist = ROOT.RooArgList()
        print "self.variables_ = %s" %self.variables_
        weight  = self.getVar(options.weight_expression)[0]
        print self.getVar(options.weight_expression)[0]
        print "weight = %s \n varlist = %s \n" % (weight, varlist)
        print options.dataset_variables
        print options.dataset_variables+[weight]

        for var in options.dataset_variables+[weight]:
            print "self.variables_ = %s" %self.variables_
            name, binning = self.getVar(var)
            print "name = %s binning = %s" %(name, binning)
        ...
        for name, fit in options.fits.iteritems():
          ...
          ## prepare SM higgs
            if not (options.prep_data or options.prep_nohiggs):
                for higgsL in higgs:
                    higgsTrees =  self.prepareTrees(higgsL,selection,options.verbose,"higgs %s trees" % higgsL)
                    self.buildRooDataSet(higgsTrees,higgsL,name,fit,categories,fulllist,weight,preselection,storeTrees)
            if not (options.prep_data or options.prep_noggh):
                for gghL in ggh:
                    gghTrees =  self.prepareTrees(gghL,selection,options.verbose,"ggH %s trees" % gghL)
                    self.buildRooDataSet(gghTrees,gghL,name,fit,categories,fulllist,weight,preselection,storeTrees)
            if not (options.prep_data or options.prep_novbf):
                for vbfL in vbf:
                    vbfTrees =  self.prepareTrees(vbfL,selection,options.verbose,"vbf %s trees" % vbfL)
                    self.buildRooDataSet(vbfTrees,vbfL,name,fit,categories,fulllist,weight,preselection,storeTrees)
            if not (options.prep_data or options.prep_novh):
                for vhL in vh:
                    vhTrees =  self.prepareTrees(vhL,selection,options.verbose,"vh %s trees" % vhL)
                    self.buildRooDataSet(vhTrees,vhL,name,fit,categories,fulllist,weight,preselection,storeTrees)
            if not (options.prep_data or options.prep_notth):
                for tthL in tth:
                    tthTrees =  self.prepareTrees(tthL,selection,options.verbose,"tth %s trees" % tthL)
                    self.buildRooDataSet(tthTrees,tthL,name,fit,categories,fulllist,weight,preselection,storeTrees)
    ...
    def getVar(self,var):
        ...
        if len(xbins) == 0 and name in self.variables_:
            ...
            #print"ok so I am  here as self.variables={}"
            print "name = %s & xbins = %s " %(name, xbins)
    ...
    def getTreesForSelection(self,dataset,selection):
        ...
        if not key in self.trees_:
            ...
            for cat in self.categories_:
                ...
                print "Keys: %s, Category name: %s, Group name: %s" %(key,catname,group)
