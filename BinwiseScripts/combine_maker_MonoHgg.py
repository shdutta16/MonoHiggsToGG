...
from templates_maker_MonoHgg import TemplatesApp
...
class CombineApp(TemplatesApp):
            ...
            option_groups=[
                ("Combine workspace options", [
                        ...
                        make_option("--observable",dest="observable",action="store",type="string",
                                    default="mgg[38,105,181]",
                                    help="Observable used in the fit default : [%default]",
                                    ),
			make_option("--observableScaleUp",dest="observableScaleUp",action="store",type="string",
			            default="massScaleUp[38,105,181]",
				    help="Observable used in the fit default : [%default]",
                                    ),
                        make_option("--observableScaleDown",dest="observableScaleDown",action="store",type="string",
                                    default="massCorrScaleDown[38,105,181]",
                                    help="Observable used in the fit default : [%default]",
                                    ),
                        make_option("--observableSmearUp",dest="observableSmearUp",action="store",type="string",
                                    default="mass[38,105,181]",
                                    help="Observable used in the fit default : [%default]",
                                    ),
                        make_option("--observableSmearDown",dest="observableSmearDown",action="store",type="string",
                                    default="massCorrSmearDown[38,105,181]",
                                    help="Observable used in the fit default : [%default]",
                                    ),
                        make_option("--observables",dest="observables",action="callback",callback=optpars_utils.Load(scratch=True),type="string",
                                    default={ 
                                "EBEE"     : "mggEBEE[38,105,181]",
                                "EBEB"     : "mggEBEB[38,105,181]"
                                              },
                        ...
                        make_option("--plot-higgs-binning",dest="plot_higgs_binning",action="callback",callback=optpars_utils.ScratchAppend(float),
                                    type="string",default=[50,0.2],
                                    help="Number of bins and width of observable for higgs model plots",
                                    ),
                        ...
                        make_option("--generate-Higgs-dataset",dest="generate_higgs_dataset",action="store_true",default=False,
                                    help="Generate higgs dataset",
                                    ),
                        ...
                        make_option("--use-higgs-datahist",dest="use_higgs_datahist",action="store_true",default=False,
                                    help="Give RooDataHist to combine instead of RooHistPdf",
                                    ),
                        ...
                        make_option("--datacard-hig-rate",dest="datacard_hig_rate",action="store",type="string",
                                    default="1",
                                    help="To increase SM higgs rate in datacard. Works only with pdfs. For RooDataHists use --expectSignal when running combine.",
                                    ),
                        ...
                        make_option("--rescale-higgs-to",dest="rescale_higgs_to",action="store",type="float",
                                    default=None,
                                    help="Change cross section of all SM Higgs to the specified value",
                                    ),
                        make_option("--mDM",dest="m_DM",action="store",type="float",
                                    default=1,
                                    help="Value of the DM mass",
                                    ),
                        make_option("--mZp",dest="m_Zp",action="store",type="float",
                                    default=10,
                                    help="Value of the DM mass",
                                    ),
                        ...
                        make_option("--add-signal",action="store_true", dest="add_signal", 
                                    default=False,
                                    ),
                        ]
                 )
    ...
    def __call__(self,options,args):
        ...
        self.save_params_.append("higgs")
        self.save_params_.append("ggh")
        self.save_params_.append("vbf")
        self.save_params_.append("vh")
        self.save_params_.append("tth")
        ...
        if options.generate_higgs_dataset:
            self.generateHiggsDataset(options,args)
        ...
        if options.fit_background:
            self.plotBackgroundAndHiggs(options,args)
	...
	# Resonant background
	higgs = options.higgs.keys()
	ggh   = options.ggh.keys()
	vbf   = options.vbf.keys()
	vh    = options.vh.keys()
	tth   = options.tth.keys()
        ...
        for signame in signals:
            ...
            for vhname in vh:
                options.vh_root_file  = vhname+".root" 
            for tthname in tth:
            ...
            print "ggh        ws : %s" % options.ggh_root_file
            print "vbf        ws : %s" % options.vbf_root_file
            print "vh         ws : %s" % options.vh_root_file
            print "tth        ws : %s" % options.tth_root_file
...
#            datacard.write("\n")
	...
	useAsimov = (len(options.fit_asimov) > 0)
		...
		print pdf.getParameters(binned).Print("v")                
    ...
    ## ------------------------------------------------------------------------------------------------------------  
    def plotBackgroundAndHiggs(self,options,args):
    ...
    def generateHiggsDataset(self,options,args):
        
        print "--------------------------------------------------------------------------------------------------------------------------"
        print "generating SM higgs datasets"
        print 
        ...
        ## book roo observable
        roowe = self.buildRooVar("weight",[])        
        weightMod = ROOT.RooFormulaVar("weightMod" ,"weightMod","@0", ROOT.RooArgList(roowe) )
                
        higgs = options.higgs.keys()

        for higname in higgs:
            print higname
        ...
        for higname in higgs:
            ...
            # In case nothing specified about the output file, set: output_file = higname.root
            options.output_file = "%s.root" % (higname)
            nameFileOutput = options.output_file
           
            ## build and import signal dataset
            for cat in fit["categories"]:
                roobs = self.getObservable(cat)
                treename = "%s_%s_%s" % (higname,options.fit_name,cat)
                print treename
                dset = self.rooData(treename,weight="%s * weight" % self.lumiScale(treename))
                #print "here"
                #if options.higgs_scalefactor_forpdf!=1:
                #    print "inside"
                #    dsetPdf = self.rooData(treename,weight="%s * weight" %options.higgs_scalefactor_forpdf,redo=True)
                #    dsetPdf.SetName("ForPdf_%s" %treename)
                #else: 
                dsetPdf=dset
                ...
                reduced = dset.reduce(RooFit.SelectVars(rootemps),RooFit.Range("fullRange"))
                reduced.SetName("higgs_%s_%s"% (higname,cat))
                reducedPdf = dsetPdf.reduce(RooFit.SelectVars(roobsArg),RooFit.Range("fullRange"))
                reducedPdf.SetName("higgsforPdf_%s_%s"% (higname,cat))

                self.workspace_.rooImport(reduced)
                self.workspace_.rooImport(reducedPdf)
                ...
                binned = reduced.binnedClone()
                binned.SetName("higgs_%s_%s"% (higname,cat))
                binnedPdf = reducedPdf.binnedClone()
                binnedPdf.SetName("higgsforPdf_%s_%s"% (higname,cat))
                if options.verbose: 
                    binned.Print()
                    binnedPdf.Print()
                self.workspace_.rooImport(binned)
                ...
                ## build RooHistPdf in roobs
                pdfDataHist = binnedPdf
                pdf=ROOT.RooHistPdf("model_higgs_%s_%s"% (higname, cat),"model_higgs_%s_%s"% (higname, cat),ROOT.RooArgSet(roobs),pdfDataHist)
                if options.verbose:
                    print "Integral higgs pdf    :", pdf.createIntegral(ROOT.RooArgSet(roobs),"templateBinning%s"%cat).getVal()
                print "Integral higgs pdf    :", pdf.createIntegral(ROOT.RooArgSet(roobs),"templateBinning%s"%cat).getVal()
                 
                ## prepare binnning: doing it here as it is faster than on the 2D pdf
                plot_higgs_binning = None
                             
                  
                #self.plotBkgFit(cat,options,reduced,pdf,roobs,"model_higgs_%s_%s_%s" % (higname,roobs.GetName(),cat),poissonErrs=False,sig=True,hig=True,logx=False,logy=False,
                              #  plot_binning=plot_higgs_binning, forceSkipBands=True)
                ...
                if options.rescale_higgs_to:
                    norm.setVal(reduced.sumEntries()*self.getHiggsScaleFactor(higname))
                else:
                    norm.setVal(reduced.sumEntries()) 
		    ...
		    print "computing energy scale"
                    ...
                    if not "shape_unc" in fit:
                        fit["shape_unc"] = {}
                    if not higname in fit["shape_unc"]:
                        fit["shape_unc"][higname] = {}
                    if not nuisName in fit["shape_unc"][higname]:
                        fit["shape_unc"][higname][nuisName] = [cat]
                    else:
                        fit["shape_unc"][higname][nuisName].append(cat)
                        
                    ecustom.replaceArg(obsCat,shiftObs)
                    epdf = ecustom.build(True)
                    unc = options.energy_scale_uncertainties.get(cat,options.energy_scale_uncertainty)
                    scaleNuis.setVal(unc)
                    dataHistShiftUp = epdf.generateBinned(ROOT.RooArgSet(obsCat),1000.,True) #prima era generateBinned
                    dataHistShiftUp.SetName("%s_dataSet_energyScale%sUp" % (pdf.GetName(),cat) )
                    pdfShiftUp = ROOT.RooHistPdf("%s_energyScale%sUp" % (pdf.GetName(),cat),"%s_energyScale%sUp" % (pdf.GetName(),cat),ROOT.RooArgSet(obsCat),
                                                 dataHistShiftUp) #prima era RooHistPdf
    ...
    ## ------------------------------------------------------------------------------------------------------------
    def getHiggsScaleFactor(self,higname):
        coup,mass = self.getCoupAndMassFromName(higname)
        ## print coup, mass
        return self.higgs_scale_factors_[coup][mass]
    ...
    def plotBkgFit(self,cat,options,dset,pdf,obs,label,blabel=None,extra=None,bias_funcs=None,poissonErrs=True,plot_binning=None,logx=True,logy=True,
      ...
      if not sig:
        ...
        obs.setRange("fullRange2", 105,180)
        obs.setRange("lowSideRange", 105,120)
        obs.setRange("highSideRange", 130,181)
        ndata = dset.sumEntries()
        sigInt = pdf.createIntegral(ROOT.RooArgSet(obs), RooFit.Range("sigRange"))
        pdfInt = pdf.createIntegral(ROOT.RooArgSet(obs),RooFit.NormSet(ROOT.RooArgSet(obs)), RooFit.Range("fullRange"))
        pdfInt2 = pdf.createIntegral(ROOT.RooArgSet(obs),RooFit.NormSet(ROOT.RooArgSet(obs)), RooFit.Range("fullRange2"))
        TotInt= ndata*pdfInt2.getVal()/pdfInt.getVal()
        SInt=ndata*sigInt.getVal()/pdfInt.getVal()
        lowSideInt = pdf.createIntegral(ROOT.RooArgSet(obs), RooFit.Range("lowSideRange"))
        highSideInt = pdf.createIntegral(ROOT.RooArgSet(obs), RooFit.Range("highSideRange"))
        SBInt=ndata*(lowSideInt.getVal()+highSideInt.getVal())/pdfInt.getVal()
        TotIntErr=sqrt(TotInt)
        sigIntErr=sqrt(SInt)
        SBIntErr=sqrt(SBInt)#ndata*(sqrt((lowSideInt.sigma(roobs)).getVal()*(lowSideInt.sigma(roobs)).getVal()+(highSideInt.sigma(roobs)).getVal()*(highSideInt.sigma(roobs)).getVal()))
        alphaErr=sqrt(sigIntErr*sigIntErr/(SBInt*SBInt) + (SInt*SInt*SBIntErr*SBIntErr)/(pow(SBInt, 4)))
        print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~NDATA: ",ndata
        print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~Full Range: ",TotInt," +/- ",TotIntErr
	...
	if hig: sig=hig
        ...
        if poissonErrs:
            ...
            print "N= %s" %hist.GetN()
            if blind:
                print "blind0 = %s and blind1 = %s" %(blind[0],blind[1])
            for ip in range(hist.GetN()):
                print ip
                print hist.GetX()[ip]-hist.GetErrorXlow(ip), "  ", hist.GetX()[ip]+hist.GetErrorXhigh(ip) 
        ...
        print "Here" , npoints
        if blind:
            print "blind0 and blind1 " ,blind[0] ,"   ", blind[1]
        #for ip in range(npoints,0,-1):
        for ip in range(npoints):
            print hresid.GetX()[ip]-hresid.GetErrorXlow(ip), "  " , hresid.GetX()[ip]+hresid.GetErrorXhigh(ip)
	    ...
	    canv = ROOT.TCanvas("hig_fit_%s" % label, "hig_fit_%s" % label)
	    legend = None
        else:
        ...
        if sig:
            ...
            ymin = fitc.interpolate(frame.GetXaxis().GetXmax())*10
        if not logy:
            ymax = ymax * 0.5
	...
	if plotHiggs:
            print "------------------------------------->",options.ws_dir
            gghfile=ROOT.TFile.Open("%s/GluGluHToGG_mA%s_13TeV.root" % (options.ws_dir,medMass),"read")
            gghws=gghfile.Get("wtemplates")
            dsetggh=gghws.data("higgs_GluGluHToGG_mA%s_13TeV_%s"% (medMass,cat))
            pdfggh=gghws.pdf("model_higgs_GluGluHToGG_mA%s_13TeV_%s"% (medMass,cat))
            
            vhfile=ROOT.TFile.Open("%s/VHToGG_mA%s_13TeV.root" % (options.ws_dir,medMass),"read")
            vhws=vhfile.Get("wtemplates")
            dsetvh=vhws.data("higgs_VHToGG_mA%s_13TeV_%s"% (medMass,cat))
            pdfvh=vhws.pdf("model_higgs_VHToGG_mA%s_13TeV_%s"% (medMass,cat))
            
            vbffile=ROOT.TFile.Open("%s/VBFHToGG_mA%s_13TeV.root" % (options.ws_dir,medMass),"read")
            vbfws=vbffile.Get("wtemplates")
            dsetvbf=vbfws.data("higgs_VBFHToGG_mA%s_13TeV_%s"% (medMass,cat))
            pdfvbf=vbfws.pdf("model_higgs_VBFHToGG_mA%s_13TeV_%s"% (medMass,cat))
            
            tthfile=ROOT.TFile.Open("%s/ttHJetToGG_mA%s_13TeV.root" % (options.ws_dir,medMass),"read")
            tthws=tthfile.Get("wtemplates")
            dsettth=tthws.data("higgs_ttHJetToGG_mA%s_13TeV_%s"% (medMass,cat))
            pdftth=tthws.pdf("model_higgs_ttHJetToGG_mA%s_13TeV_%s"% (medMass,cat))
	    ...
	    dsettemp = dset.Clone()
            ...
            #compute integral signal region
	...
	### Apply exact formatting wanted
        #l1b=ROOT.TLatex(0.145,0.87,"#bf{CMS} #it{Preliminary}");
        ...
        #l2b=ROOT.TLatex(0.75,0.94,"35.9 fb^{-1} (13 TeV)")
        ...
        #l2b.Draw("same")
        #
        #l3b=ROOT.TLatex(0.145,0.79,"DM+H(#gamma#gamma)")
        ...
        #if cat=="met130":
        #    l4b=ROOT.TLatex(0.145,0.74,"p_{T}^{miss} > 130 GeV")
        #    l4b.SetTextSize(0.03)
        #    l4b.SetTextAlign(12)
        #    l4b.SetNDC()
        #    l4b.SetTextFont(42)
        #    l4b.Draw("same")
        #else:
        #    l4b=ROOT.TLatex(0.145,0.74,"50  GeV < p_{T}^{miss} < 130 GeV")
        #    l4b.SetTextSize(0.03)
        #    l4b.SetTextAlign(12)
        #    l4b.SetNDC()
        #    l4b.SetTextFont(42)
