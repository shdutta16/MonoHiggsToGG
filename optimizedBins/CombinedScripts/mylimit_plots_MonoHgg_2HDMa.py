...
class LimitPlot(PlotApp):
    ...
    def __call__(self,options,args):
        ...
        if options.model=="BARY":
            flist = [ "%s/higgsCombineMonoHgg%s_sig_ZpBaryonicCombined.%s.ZpBaryonic_mChi%s.root" % (options.input_dir,options.suffix, options.method,options.mDM)] #MARGARET
        if options.model=="2HDM": 
            flist = [ "%s/higgsCombineMonoHgg%s_sig_2HDMCombined.%s.2HDM_mA0%s.root" % (options.input_dir,options.suffix, options.method,options.mDM)] #MARGARET
        if options.model=="2HDMa": 
            flist = [ "%s/higgsCombineMonoHgg%s_sig_2HDMaCombined.%s.2HDMa_ma%s.root" % (options.input_dir,options.suffix, options.method,options.mDM)] #Debu
        ...
        for fname in flist:
            ...
            print coup
            if options.model=="BARY":
                coup = coup[2].replace("ZpBaryonic_mChi","") #MARGARET
            if options.model=="2HDM":
                coup = coup[2].replace("2HDM_mA0","")        #MARGARET
            if options.model=="2HDMa":
                coup = coup[2].replace("2HDMa_ma","")        #MARGARET
            print coup
        ...
        if len(options.couplings) == 0:
            if options.model=="BARY": graphs = self.open("%s/graphs_ZpBaryonic_%s_%s.root" % (options.input_dir,options.method,options.mDM),"recreate") #MARGARET
            if options.model=="2HDM": graphs = self.open("%s/graphs_2HDM_%s_%s.root" % (options.input_dir,options.method,options.mDM),"recreate") #MARGARET
            if options.model=="2HDMa": graphs = self.open("%s/graphs_2HDMa_%s_%s.root" % (options.input_dir,options.method,options.mDM),"recreate") 
            #graphs = self.open("%s/graphs_%s%s.root" % (options.input_dir,options.label,options.method),"recreate")
        else:
            if options.model=="BARY": graphs = self.open("%s/graphs_ZpBaryonic_%s_%s.root" % (options.input_dir,"_".join(options.couplings),options.method),"recreate") #MARGARET
            if options.model=="2HDM": graphs = self.open("%s/graphs_2HDM_%s_%s.root" % (options.input_dir,"_".join(options.couplings),options.method),"recreate") #MARGARET
            if options.model=="2HDMa": graphs = self.open("%s/graphs_2HDMa_%s_%s.root" % (options.input_dir,"_".join(options.couplings),options.method),"recreate") 
    ...
    def plotLimit(self,options,coup,tfile):
        if options.model=="BARY":
            tfile_xsecTheo = self.open("theory_ZPBaryonic_vs_mDM1.root") #MARGARET
            theo = tfile_xsecTheo.Get("theory_vs_mDM1")                  #MARGARET
        #if options.model=="2HDM":
            #tfile_xsecTheo = self.open("theory_2HDM_vs_mA300.root")      #MARGARET
            #theo = tfile_xsecTheo.Get("xsec_2HDM")                       #MARGARET
        ...
        print "-------------------------------------------------------"
        observed   = scaleGraph2(observedOrig,1)   #MARGARET
        expected68 = scaleGraph2(expected68Orig,1) #MARGARET
        expected95 = scaleGraph2(expected95Orig,1) #MARGARET
        if options.cleanup:
        ...
        if options.model=="BARY":
            basicStyle = [["SetMarkerSize",0.6],["SetLineWidth",3],
                           ["SetTitle",";M_{Z`} (GeV);95%% C.L. #sigma(pp#rightarrow Z'_{B}H #rightarrow#chi#chi#gamma#gamma ) (%s)" % unit]] #MARGARET
        if options.model=="2HDM":
            basicStyle = [["SetMarkerSize",0.6],["SetLineWidth",3],
                           ["SetTitle",";M_{Z`} (GeV);95%% C.L. #sigma(pp#rightarrow AH #rightarrow#chi#chi#gamma#gamma ) (%s)" % unit]] #MARGARET
        #if options.model=="2HDMa":
         #   basicStyle = [["SetMarkerSize",0.6],["SetLineWidth",3],
          #                 ["SetTitle",";M_{A} (GeV);95%% C.L. #sigma(pp#rightarrow aH #rightarrow#chi#chi#gamma#gamma ) (%s)" % unit]] #MARGARET

        if options.model=="2HDMa":
            basicStyle = [["SetMarkerSize",0.6],["SetLineWidth",3],
                           ["SetTitle",";M_{A} (GeV);95% C.L. asymptotic limit on #mu=(#sigma/#sigma_{th})"]] #Debabrata
        ...
        #commonStyle = [[self.scaleByXsec,coup],"Sort"]+basicStyle #MARGARET -- FIXME -- comment out SORT
        ...
        theoryStyle   = [["SetMarkerSize",0]]+basicStyle
       
        #style_utils.apply( theo,       [["colors",ROOT.kBlue],["SetLineStyle",5],["SetName","theo"]]+theoryStyle ) #MARGARET 
        ...
        if options.model=="BARY": 
            canv  = ROOT.TCanvas("limits_ZPBaryonic_mDM%s%s%s"%(options.mDM,options.model,options.addName),"limits_ZPBaryonic_mDM%s%s%s"%(options.mDM,options.model,options.addName)) #MARGARET
        if options.model=="2HDM":
        ...
        legend = ROOT.TLegend(0.68,0.75,0.88,0.9)
        expected95.Draw("AE3")        
        if options.model=="BARY": expected95.GetXaxis().SetRangeUser(10,10000) #MARGARET
        if options.model=="2HDM": expected95.GetXaxis().SetRangeUser(600,4000) #MARGARET
        if options.model=="2HDMa": expected95.GetXaxis().SetRangeUser(200,1000)
        expected95.GetYaxis().SetRangeUser(0.1,5000)
        expected95.GetYaxis().SetTitleSize(0.04)
        ...
        if options.model=="BARY": legend.AddEntry(theo,"Z' baryonic: q_{q} = 0.25, m_{#chi} = 1 GeV","l") #MARGARET
        #if options.model=="2HDM": legend.AddEntry(theo,"2HDM #m_{A} = 300 GeV","l") #MARGARET
        legend.AddEntry(expected,"Expected limit","l")
        legend.AddEntry(expected68," #pm 1 s.d.","f")
        legend.AddEntry(expected95," #pm 2 s.d.","f")
        legend.AddEntry(l," Theory.","l")
        if options.unblind:
          # observed.Draw("PL")
          observed.Draw("L")
    ...
    def scaleByXsec(self,graph,coup):
        ...
        if not self.options.fixed_x_section:
            if not coup in self.xsections_:
                xsec = 1
                print("Using cross section xsec = 1")
        ...
        for ip in range(graph.GetN()):
            iscale = scale
            if not self.options.fixed_x_section:
                #iscale = xsec.Eval(xvals[ip]) #MARGARET
                iscale = 1
            elif rescaleFunc: 
