...
namespace flashgg {
    ...
    std::vector<edm::Ptr<Electron> > selectLooseElectrons( const std::vector<edm::Ptr<flashgg::Electron> > &ElectronPointers,
                                                            ...
                                                            double dRPhoLeadEleThreshold, double dRPhoSubLeadEleThreshold )
    {
        ...
        for( unsigned int ElectronIndex = 0; ElectronIndex < ElectronPointers.size(); ElectronIndex++ ) {
            ...
            float sieie_cut = 0.011;
            float detacut = 0.00477;
            float dphicut = 0.222;
            float hoecut = 0.298;
            float relisocut = 0.0994;
            float ooemoopcut = 0.241;
            float dxycut = 0.05;
            float dzcut = 0.10;
            int misshcut = 1;  
            // + conversion veto 
    ...
    std::vector<edm::Ptr<Electron> > selectMediumElectrons( const std::vector<edm::Ptr<flashgg::Electron> > &ElectronPointers,
                                                            const std::vector<edm::Ptr<reco::Vertex> > &vertexPointers,
                                                            Ptr<flashgg::DiPhotonCandidate> dipho, float rho, double ElectronPtThreshold,
                                                            double dRPhoLeadEleThreshold, double dRPhoSubLeadEleThreshold )
    {
        ...
        for( unsigned int ElectronIndex = 0; ElectronIndex < ElectronPointers.size(); ElectronIndex++ ) {
            ...
            // hardcoded cuts
            // https://twiki.cern.ch/twiki/bin/view/CMS/CutBasedElectronIdentificationRun2#Spring15_selection_25ns
            float sieie_cut = 0.00998;
            float detacut = 0.00311;
            float dphicut = 0.103;
            float hoecut = 0.253;
            float relisocut = 0.0695;
            float ooemoopcut = 0.134;
            float dxycut = 0.05;
            float dzcut = 0.10;
            int misshcut = 1;  
            // + conversion veto 
