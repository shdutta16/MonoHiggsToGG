...
# m2 = mZp
for m1 in 1; do
  for m2 in {10,20,50,100,200,300,500,1000,2000,10000}; do
    card=dataCard_sig_${model}_mZP${m2}_${m1name}${m1}_13TeV.txt
    binary=$(echo $card | sed 's%.txt$%.root%')
    signame=$(echo $card | sed 's%.*MonoHgg_%%; s%.txt%%')
    echo $signame

    namestr=${label}_mZP${m2}_${m1name}${m1}
    ...
    log=combine_log_${method}_${label}_mZP${mass}.log
    set -x
