...
void skim(TString path, TString path2,TString sample){
  ...
  */
  ...
  for (UInt_t entry = 0; entry < intree->GetEntries(); entry++)
  {
    ...
    const Bool_t passKinematics   = (t1pfmetCorr>130) ? (pt1>0.5*mgg && pt2>0.25*mgg && ptgg>90 && mgg>=100 && mgg<=200): (pt1>0.45*mgg && pt2>0.25*mgg && ptgg>40 && mgg>=100 && mgg<=200);
    //const Bool_t passKinematics   = (pt1>0.5*mgg && pt2>0.25*mgg && ptgg>90 && mgg>=100 && mgg<=200);
    //const Bool_t passKinematics   = (pt1>0.45*mgg && pt2>0.25*mgg && ptgg>40 && mgg>=100 && mgg<=200);
