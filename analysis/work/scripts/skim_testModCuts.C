...
void skim(TString path, TString path2,TString sample){
  ...
  */
  ...
  // ----------------------------------------------------------------
  TFile * outfile = new TFile(Form("%s/%s_skimmedtree.root",path2.Data(),sample.Data()),"RECREATE");
  ...
  for (UInt_t entry = 0; entry < intree->GetEntries(); entry++)
  {
    ...
    const Bool_t passKinematics   = (pt1>0.5*mgg && pt2>0.25*mgg && ((t1pfmetCorr<=150 && ptgg>40) || (t1pfmetCorr>150 && ptgg>90)) && mgg>=100 && mgg<=200);
    //const Bool_t passKinematics   = (pt1>0.45*mgg && pt2>0.25*mgg && ptgg>40 && mgg>=100 && mgg<=200); // LowMET
    ...
    const Bool_t passDphiCutsJ    = (mindphijmet >= 0.5);
    const Bool_t passDphiCutsGG   = (dphiggmet >= 2.1);
    //const Bool_t passDphiCuts     = true;
    const Bool_t passDphiCuts     = (dphiggmet>=2.1 && mindphijmet>=0.5 /* && max_dphi_JetMET <= 2.7 */);
