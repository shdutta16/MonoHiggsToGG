...
class LimitPlot(PlotApp):
    ...
    def __call__(self,options,args):
        ...
        for fname in flist:
            ...
            coup = coup[1].split(".")
            print coup
            coup = coup[2].replace("ZpBaryonic_mChi","")
            print coup
        ...
        if len(options.couplings) == 0:
            graphs = self.open("%s/graphs_ZpBaryonic_%s_%s.root" % (options.input_dir,options.method,options.mDM),"recreate") #MARGARET
            #graphs = self.open("%s/graphs_%s%s.root" % (options.input_dir,options.label,options.method),"recreate")
        else:
            graphs = self.open("%s/graphs_ZpBaryonic_%s_%s.root" % (options.input_dir,"_".join(options.couplings),options.method),"recreate") #MARGARET
    ...
    def plotLimit(self,options,coup,tfile):
        ...
        expected95.Draw("AE3")        
        expected95.GetXaxis().SetRangeUser(10,1000)
        ...
        legend.AddEntry(theo,"Z' baryonic: q_{q} = 0.25, m_{#chi} = 1 GeV","l") #MARGARET
        ...
        self.format(canv,options.postproc)
        canv.SaveAs("%slimits_ZPBaryonic_mDM%s.png"%(options.outdir,options.mDM))
