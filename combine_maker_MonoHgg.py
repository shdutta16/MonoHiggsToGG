...
#            datacard.write("\n")
	...
	if plotHiggs:
            ...
            gghfile=ROOT.TFile.Open("%s/GluGluHToGG_13TeV.root" % (options.ws_dir),"read")
            gghws=gghfile.Get("wtemplates")
            dsetggh=gghws.data("higgs_GluGluHToGG_13TeV_%s"% (cat))
            pdfggh=gghws.pdf("model_higgs_GluGluHToGG_13TeV_%s"% (cat))
            
            vhfile=ROOT.TFile.Open("%s/VHToGG_13TeV.root" % (options.ws_dir),"read")
            vhws=vhfile.Get("wtemplates")
            dsetvh=vhws.data("higgs_VHToGG_13TeV_%s"% (cat))
            pdfvh=vhws.pdf("model_higgs_VHToGG_13TeV_%s"% (cat))
            
            vbffile=ROOT.TFile.Open("%s/VBFHToGG_13TeV.root" % (options.ws_dir),"read")
            vbfws=vbffile.Get("wtemplates")
            dsetvbf=vbfws.data("higgs_VBFHToGG_13TeV_%s"% (cat))
            pdfvbf=vbfws.pdf("model_higgs_VBFHToGG_13TeV_%s"% (cat))
            
            tthfile=ROOT.TFile.Open("%s/ttHJetToGG_13TeV.root" % (options.ws_dir),"read")
            tthws=tthfile.Get("wtemplates")
